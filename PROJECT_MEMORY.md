...
- **2026-05-10 已对齐国赛官网规则**：详见 `docs/RULE_ASSUMPTIONS.md`。当前 core 存在 1 个 P0 合规缺口（不允许吃本方棋子）需修复，详见 `PROJECT_PHASES.md` 阶段 R-0。
...
- **吃子**：目标格有棋子（**含本方**）则吃掉；吃本方子是合法策略。⚠️ 当前 core 实现与此规则不符。
...
- ⚠️ **缺失**：开局录入（对方布局）、7 盘制比赛流程、崩溃自救（详见 `PROJECT_PHASES.md` 阶段 R-1 / R-2 / R-3）。
...
2. 确认是否优先做阶段 R-0（吃本方棋子合规修复）。这是合规 P0，且会影响所有后续 bench 数据的可信度。
3. R-0 完成后，依次评估 R-1（开局录入 GUI）、R-2（7 盘制比赛模式）、R-3（崩溃自救）。
4. 完成 R-x 之后再决定是否回到原阶段 6 Expectimax 强化主线（R-0 修复后 ExpectimaxAI 的强度评估可能翻盘）。
