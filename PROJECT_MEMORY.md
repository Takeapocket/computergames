...
- **2026-06-13 项目转型为长期研究项目**：2026 赛季已结束（省赛 2026-06-07 已举行）。经用户确认，项目定位改为**纯研究**：4 分钟包干、离线部署、封版纪律解除；接受 numpy/torch（本机有 NVIDIA GPU）与必要时的 Rust/C 扩展。路线为"先地基后双线"：先做性能基线/持久 Elo 天梯/真实棋谱复盘与骰子取证，再并行推进经典搜索线（expectiminimax + TT + Star 剪枝 + 终局求解）与自对弈学习线（MCTS 底盘 + 调参 + 价值网络）。设计见 `docs/superpowers/specs/2026-06-13-research-transition-design.md`，路线图见 `PROJECT_PHASES.md` §2.5。**硬约束：C 盘只剩约 53GB，训练数据/模型权重/缓存一律放 E 盘；项目本体迁移到 `E:\computergame`；README 及对外文档不写比赛结果信息。** release/v1.0 与 P14 默认配置保留为历史基线和天梯锚点。
- **2026-06-13 R-P1 本地地基脚手架已落地（未执行 R-P0 外部动作）**：新增 `scripts/perf_probe.py`、`scripts/ladder.py`、`scripts/replay_analyze.py`、`scripts/dice_forensics.py`、`requirements-research.txt`、`reports/rp1_foundation_bootstrap_20260613.md` 与执行计划 `docs/superpowers/plans/2026-06-13-research-foundation-plan.md`。当前脚手架只在 C 盘工作区提供可测入口：perf probe 输出 match step timing、rollout root visits 与 rollout 决策窗口内 clone / legal move / GreedyAI / RNG 微计数；ladder 以 P14 `kind="rollout"` + `RELEASE_DEFAULT_ROLLOUT_KWARGS` 作为 1500 锚点，支持双人 probe、多人 round-robin、Elo 不确定度与 Markdown 报告；审查 follow-up 后 CLI 不再默认写 C 盘仓库内 `data/ladder`，必须显式传 `--output-dir` 或设置 `CG_RESEARCH_DATA_DIR`，且会拒绝混用已有非空 `games.jsonl`。replay/dice 脚本可解析 GameRecord/MatchRecord，支持记录走法 vs 当前 P14 推荐的逐步对比，并明确 `source` 不是独立骰子来源字段。未执行 PR、迁移到 E 盘、重建 `.venv`、全局 pip 配置、依赖安装、commit、push 或删除 C 盘旧目录。验证：新增测试组合 24 passed；全量 `pytest -q` 为 886 passed in 71.78s。
- **2026-06-14 R-P1 trusted apply fast path 已完成（行为等价小步优化）**：`GameState.apply_move()` 仍保留终局、当前方、骰子映射、合法性与 canonical capture 校验；新增私有 `_apply_known_legal_move()` 只供“刚从当前状态 `legal_moves()` 得到的 Move”内部热路径使用，并保留终局/当前方便宜检查。`RolloutAI._sample_move_score()`、`_playout()`、`RolloutPairedAI.choose_move()` 与 `_playout_with_rng()` 已改用该 helper，避免 rollout 内部已知合法走法再次走 `_find_matching_legal_move()` 的二次合法步生成。新增 public canonical capture、private fast path 等价/拒绝边界、RolloutAI 固定 seed 推荐/root stats/RNG progression characterization 测试。perf probe 见 `reports/rp1_trusted_apply_fastpath_probe_20260614.json`，报告见 `reports/rp1_trusted_apply_fastpath_20260614.md`；本轮只记录 post-change smoke（8 samples、1900 root visits、0 illegal/crash/timeout），不声称 5x 提速。验证：`tests/test_game_state.py` + `tests/test_rollout_ai.py` 为 36 passed；全量 `pytest -q` 为 926 passed in 73.99s。默认 AI、GUI、release/v1.0、P14 参数和 core 公开规则语义未改变。
- **2026-06-14 research review follow-up 已完成本地修复（未 commit/push/迁移）**：修复 Superpowers 并行审查指出的 C 盘默认输出红线与 Important 问题：`scripts/ladder.py` CLI 现在要求 `--output-dir` 或 `CG_RESEARCH_DATA_DIR`，拒绝复用非空 `games.jsonl`，并在每局 JSONL 写入 red/blue 的 kind/kwargs/signature；`scripts/dice_forensics.py` 的 threat coincidence 改为“非行动方已有一步到角威胁时，行动方当前骰子是否可吃威胁子”，并输出 chi-square p-value 与 Cramer's V；`RolloutPairedAI` 增加固定 seed characterization；`_apply_known_legal_move()` 增加 O(1) stale guard；`mcts_choose_move()` 透传 leaf playout 参数；`.gitignore` 增加研究缓存/权重防线；`docs/PROJECT_BRIEF.md`、stop list、R-P1/R-P2B 报告同步研究模式与 E 盘输出口径。验证：受影响模块测试 103 passed；全量 `pytest -q` 为 933 passed in 75.00s。未执行 `git commit`、`git push`、E 盘 clone、依赖安装、迁移或删除 C 盘旧目录。
- **2026-06-13 R-P2A ExpectimaxV2 经典搜索线已开闸到 Task 5 Step 3（未改默认 AI）**：新增执行计划 `docs/superpowers/plans/2026-06-13-rp2a-expectimax-v2-plan.md`，明确技术假设、判停条件、TT/move ordering/迭代加深/Star1-Star2 的顺序和禁改边界。`ai/expectimax_v2.py` 已落稳定 state key、包含 node type / perspective / depth / dice / evaluator version 的 transposition key、显式搜索值域常量与 bounds 检查；并新增默认关闭的 per-search transposition table、`move_ordering`、`iterative_deepening`、`chance_pruning` 与搜索统计。TT 只写入完整 exact 值，timeout/部分搜索结果不写入；ordering 优先直接胜、敌子捕获、推进，并保持相同排序分数的原始 legal 顺序；迭代加深只接受完整完成深度，超时后返回最后完成深度结果，无完成深度时回退合法步。Star1/Star2 前置值域门禁已补强：`expectimax_v2_require_score_in_bounds()` 对 leaf、terminal、no-move、chance、turn 与 TT cache hit 值执行显式 bounds 检查；`chance_pruning="star1"` 默认关闭，小深度与未剪枝值/选招等价；在 root 候选走法上，`star1` 已利用 incumbent root score 做 chance-node 上界剪枝，严格 `<` 保留并列，实际跳过剩余骰子时记录 `last_search_stats.chance_prunes`，剪枝 bound 不作为 exact TT 值存储。报告见 `reports/rp2a_star_pruning_entry_20260613.md`。`choose_move()` 默认行为、core、GUI、release/v1.0 和 P14 rollout 默认均未改变；完整递归 Star1/Star2 interval pruning 仍未实现。验证：`tests/test_expectimax_v2.py` + 签名测试为 32 passed；全量 `pytest -q` 为 913 passed in 73.30s。
- **2026-06-13 R-P2B MCTS leaf playout L0 已开闸并完成 ladder smoke（未改默认 AI）**：`ai/mcts.py` 新增默认关闭的 `leaf_policy`，`static` 保持旧静态 leaf，显式 `playout` 才从叶子局面执行最多 `leaf_playout_turns` 个随机半回合；终局/无合法步返回 `WIN_VALUE/-WIN_VALUE`，截断后回到当前静态 evaluator，并用 undo 还原搜索局面。`MCTSAI.max_step_time_ms` 现在等于 `time_limit_ms`，使 `play_one_game()` timeout telemetry 能识别 MCTS 预算；`ai_version_signature()` 记录 `leaf_policy` / `leaf_playout_turns`，`build_ai("mcts_eval_v1")` 可透传这些参数。`scripts/ladder.py` 新增 `--players-config` JSON 玩家配置入口与 `--max-turns` 运行边界，R-P2B smoke 配置见 `reports/rp2b_mcts_leaf_playout_ladder_players_20260613.json`。小样本 ladder：`games=6`、`seed=62013`、`layout=balanced_v1`、`max_turns=40`，当时输出到仓库 ignored 目录 `data/ladder/rp2b_mcts_leaf_playout_l0_20260613_v2/`；审查 follow-up 后该目录不得作为默认目标，后续运行必须显式指定外部输出目录或 `CG_RESEARCH_DATA_DIR`。`illegal_moves=0`、`crashes=0`、`timeouts=0`；rating 为 `mcts_static_z_l0=1523.576±156.525`、`p14_default=1502.675±156.525`、`mcts_playout_z_l8=1473.749±156.525`。该样本只证明可进入持久天梯，暂无 playout 叶子优于 static 叶子的正信号，不能作为棋力结论；默认 AI、GUI、release/v1.0、P14 rollout 默认均未改变。验证：`tests/test_mcts.py` 为 25 passed，`tests/test_ladder.py` 为 12 passed，全量 `pytest -q` 为 921 passed in 72.91s；Superpowers code review 无 Critical/Important，已修正报告测试计数。
...
1. 先读取本文件 + `PROJECT_PHASES.md` §2.5 长期研究路线图。
2. 项目已于 2026-06-13 转入**长期研究模式**；本体计划迁移至 `E:\computergame`，但当前会话尚未执行迁移，C 盘工作区仍是当前实际工作区。R-0~R-4、S1~S4、P1~P14 全部闭环，GUI/release 默认仍是 `rollout` + P14 参数（64 rollout / move、2000ms step deadline、epsilon 0.05、80ms deadline safety），作为研究天梯的 1500 锚点。
3. 硬约束：C 盘红线（大文件一律 E 盘）；README 不写比赛结果；棋力结论只认天梯/批量对战数据；旧失败路线重开必须有新技术假设（`reports/ai_experiment_stop_list_20260518.md` 重开条件有效）。
4. 当前研究主线按 R-P1 地基（perf_probe、Elo 天梯、replay_analyze + dice_forensics）→ R-P2A/R-P2B 双线推进；每个研究项先写技术假设与门禁再动手。
5. 待用户提供：省赛比赛机上的真实棋谱 JSON（拷回 `records/`，供复盘与骰子取证）。
