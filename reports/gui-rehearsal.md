...
真实 Tk GUI 的人工视觉/交互演练仍需按 §4 填表后，才可判定 S2 完整闭环。
...
当前表格尚未填写，因此 S2 只能判定为 headless 自动演练完成，不能判定为完整现场手测完成。
...
| 步骤 | 预期 | 结果 |
|---|---|---|
| 启动程序 | 棋盘 + 控件正常渲染；菜单出现"模式"项 | _________ |
| 菜单 → 模式 → 比赛模式 | 弹颜色 + 角色 dialog | _________ |
| 选红方 / 甲方 / 确认 | 进入第 1 盘 setup；MatchModePanel 显示比分 0:0、第 1 盘、本盘先手"我方"、身份"甲方" | _________ |
| 录入对方布局 → 确认开局 | playing 阶段；状态栏"开局已确认，请录入第一轮骰子" | _________ |
| 录骰子 → 选合法走法 → 执行 | 棋盘动；自动保存写入 | _________ |
| 连续操作至我方到达蓝方角点或吃光蓝方 | 弹"本盘 我方 胜" → 点继续 → 进入第 2 盘 setup | _________ |
| 我方布局 sticky 保留 / 对方布局清空 | OpeningPanel 显示我方按钮选中、对方区域空 | _________ |
| 重复连胜 4 盘 | 第 4 盘结束弹"本轮结束！我方 胜出，最终比分 我方 4 — 对方 0" | _________ |

### 4.2 4:3 决胜

| 步骤 | 预期 | 结果 |
|---|---|---|
| 启动新轮，我方甲红 | 同 §4.1 | _________ |
...
| 步骤 | 预期 | 结果 |
|---|---|---|
| 第 3 盘 playing 中关掉程序（任务管理器结束 python.exe） | `replays/auto_save.json` + `auto_save_match.json` 都存在 | _________ |
| 重启 `scripts/run_gui.py` → 弹"恢复未完成对局" → 选是 | 进入第 3 盘 playing；比分 + 局面 + 计时一致 | _________ |

### 4.4 盘间崩溃恢复

| 步骤 | 预期 | 结果 |
|---|---|---|
| 完成第 2 盘 → 进入第 3 盘 setup 时关程序 | `replays/auto_save_match.json` 存在；`auto_save.json` 已被 finalize 清理 | _________ |
| 重启 → 选是 | 进入第 3 盘 setup；比分保留 a:b | _________ |

### 4.5 误操作恢复

| 步骤 | 预期 | 结果 |
|---|---|---|
| playing 中走一步 → 点悔棋 | 走法撤回；状态栏提示已悔棋；计时不回退 | _________ |
| 没走过棋时点悔棋 | 状态栏"当前没有可悔棋的走法" | _________ |
| 跨盘悔棋（第 2 盘没走过时点悔棋） | 同上，不可悔到第 1 盘 | _________ |

### 4.6 整轮结束后操作

| 步骤 | 预期 | 结果 |
|---|---|---|
| 4 胜后弹整轮结束 dialog → 关闭 | GUI 留在 playing 阶段最后一手 | _________ |
| 菜单 → 模式 → 调试模式 | 切回 debug，match 字段隐藏 | _________ |
| 菜单 → 模式 → 比赛模式 | 弹颜色 + 角色 → 新一轮 | _________ |
...
至少手动跑通一轮 4 胜流程 ⏳
   → scenario_4_0 + scenario_4_3 已自动化覆盖状态机；§4 真实 GUI 手测表待操作员填写
误操作可通过悔棋或加载棋谱恢复 ✅
...
S2 headless 自动演练已闭环；完整 S2 仍缺 §4 真实 GUI 手动表：
...
- ⏳ `reports/gui-rehearsal.md` §4 真实 GUI 手动演练表待填写

**下一步**：先补齐 §4 真实 Tk GUI 手动演练记录；随后进入
`PROJECT_PHASES.md` §S3 AI 低风险清理与 harness 工程化（P2，不阻塞 S4 封版）。
