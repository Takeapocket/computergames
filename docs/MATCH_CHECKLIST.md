...
- **本程序假设**：现场无统一平台、无网络；裁判实体投掷骰子，操作员录入骰子和对方走法。
- **数据流**：骰子录入 → 程序输出我方建议走法 → 操作员执行 → 录入对方走法 → 重复。
...
| 步骤 | 操作员动作 | 程序响应 |
|---|---|---|
| 1 | 等裁判投骰子 → 在骰子输入框填入点数 → 回车 | 显示骰子数 + 合法走法列表 |
| 2 | 看 AI 推荐 + 合法走法 → 在列表选一条 | 高亮该走法的起点 / 终点 |
| 3 | 点"执行"按钮 | 棋盘应用走法，状态栏提示"已执行：…"；自动保存（每步） |
| 4 | 告知对方本方走法（规则义务） | — |
| 5 | 等对方在他们的程序操作 → 对方告知本方他们的走法 | — |
| 6 | 等裁判投骰子（这是对方的回合骰）→ 程序录入骰子 → 在合法走法里点对方实际走的那条 → 执行 | 棋盘应用对方走法；状态栏 source=opponent |
