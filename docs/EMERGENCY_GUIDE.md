...
4. 处理时间 ≤30 秒能搞定的，先处理；搞不定的立即举手叫裁判。若裁判宣布暂停，立即点 GUI 的"暂停计时"。
...
4. 除计时面板的裁判确认超时判负按钮外，当前 GUI 没有通用手动补比分入口；如果程序整体仍可继续运行，按裁判确认的真实比分做纸面记录，重新进入比赛模式开始后续对局，赛后再补整理棋谱。
...
- 计时器显示我方剩余 0:00。
- 默认状态栏显示"计时提示：X 方已到 0。未自动判负，请以裁判判定为准。"

### 默认处理：只提示，不自动判负

默认比赛模式不让程序自行判负。计时到 0 后：

- 棋局仍保持 playing，不自动 finalize。
- 合法走法仍可继续录入。
- MatchModePanel 显示超时提示。
- 计时面板会启用对应的"裁判判红方超时负" / "裁判判蓝方超时负"按钮。
- 是否判负以现场裁判为准。

操作员动作：
1. 立即报告裁判。
2. 如果裁判判某方超时负，点击计时面板中对应的"裁判判 X 方超时负"按钮，二次确认后程序会写入 `reason="timeout"`、推进比分并进入下一盘 setup。
3. 如果裁判允许继续，不要点击超时判负按钮；点"暂停计时"或按裁判要求恢复计时，再继续录入。

### 裁判要求程序自行计时时

进入比赛模式时勾选"程序自动超时判负"。该模式下计时到 0 会走旧自动处理路径：

- `_handle_timeout(超时方)` 被定时器周期调用触发。
- 走 `_finalize_match_game(胜方, reason="timeout")`。
- 自动保存 match → 清单盘 auto_save → 弹本盘结束 dialog → 自动进入下一盘 setup。

只有现场明确要求双方程序自行计时时才打开这个选项。
...
赛中遇到对方时间归零，默认也只是提示；是否判胜仍以裁判为准。裁判确认后再点击对应超时判负按钮。若已开启"程序自动超时判负"，程序会自动判我方胜并按 §盘间正常推进。
...
2. 让程序新建对局：菜单 → 模式 → 调试模式（这会清掉单盘 auto_save 和 match）。
   - **注意**：这会丢失当前盘的恢复能力。
3. 除计时面板的裁判确认超时判负按钮外，当前 GUI 不能手动 append 已结束盘或补比分；把真实比分交给裁判纸面确认，赛后再修复棋谱文件。
...
| 录入错误（已执行） | 悔棋 | 加载手动快照 |
| 多步错乱 | 加载快照 | 本盘判负 |
| 计时即将归零 | 报告裁判 | 裁判确认后点对应超时判负按钮；只有已开启自动判负时才让程序自动处理 |
