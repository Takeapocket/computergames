...
10. 掷出骰子点数 `d` 时，如果己方编号 `d` 的棋子存活，则必须移动该棋子；如果该棋子已被吃掉，则可选择编号距离 `d` 最近的存活棋子；如果两侧距离相同，则两个棋子都可选。
    - 骰子来源由双方协商或裁判要求决定；程序支持 GUI 内"程序掷骰"和手动录入两种方式。该能力不改变 core 骰子映射和合法走法规则。
...
- **单方时限**：每盘每方 4 分钟，包干计时，超时判负。
  ...
  - GUI 默认只提示超时，不自动判负；现场判罚以裁判为准。若裁判要求双方程序自行计时判负，进入比赛模式时勾选"程序自动超时判负"。
...
- `--total-seconds 600` 或比赛模式弹窗的"单方时限（秒）"已能复用 timer_panel 跑快棋，无需大改。
- 决赛触发概率低，赛前一周再补。

## 历史问题：3 分钟版本的考量

历史上有过每方 3 分钟的版本（180 秒）。当前已确认本届为 4 分钟。如赛前发现规则变更，可在比赛模式弹窗里把"单方时限（秒）"改为 180；命令行也可启动时加 `--total-seconds 180`，无需改代码：
