...
Claude 原计划的门槛制思路保留，但执行顺序和工程纪律按当前仓库状态修订：

```text
S0 基线验证
→ S1 R-3 崩溃自救（每步自动保存 + 启动恢复）
→ S2 R-1 完整灵活开局录入（预设选择 + 可视化编辑 + 自定义布局保存）✅
→ S3 R-2 七局四胜（甲乙先手、红蓝执子、比分）
→ S4 GUI 全流程演练与测试补齐
→ S5 低风险 AI followup（非阻塞）
→ S6 赛前冻结
```

执行约束：

- 不默认执行 `git commit`、`git push`、`git reset` 或创建分支；需要时必须由用户明确要求。
- 每个阶段先写相关测试，再实现最小代码，通过后再进入下一阶段。
- 赛场功能优先于 AI 强度；R-2 未完成前，不推进 Expectimax 主线。
- `greedy_risk` 继续作为默认参赛 AI；Expectimax 只作为实验入口，不能成为封版阻塞项。
- R-1 的最终验收包含完整灵活布局编辑器；可先交付比赛闭环，但不能把编辑器与自定义布局保存推到赛后。
- R-2 必须显式区分“甲/乙先手序列”和“红/蓝执子颜色”，不能把甲方等同于红方。
- 详细执行计划见 `docs/superpowers/plans/2026-05-11-competition-sprint.md`。
...
### 阶段 R-1：开局录入 GUI（P1）✅ 已完成 (2026-05-11)
...
**实现摘要**：新增 `ai/opening_layouts.py`、`gui/opening_panel.py`；`gui/board_widget.py` 支持出发区编辑模式；`gui/main_window.py` 默认进入 setup 阶段，确认开局后进入 playing 阶段并写入 `GameRecord.metadata`。`ai/match.py` 暴露新 preset 入口但保留原 `STARTING_LAYOUT_ID` 语义。
...
| 2 最小 GUI | ✓ 完成 | |
| 3 棋谱/计时/比赛模式 | ⚠️ **部分完成** | R-1 / R-3 已完成；R-2 七盘制仍需补完 |
...
- R-2 是赛前必须
- R-0-followup（stuck_penalty 死代码清理 + ai/risk.py self-capture 扩展）可以与 R-2 并行，不阻塞主线
